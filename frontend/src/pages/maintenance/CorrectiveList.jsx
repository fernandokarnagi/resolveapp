...
export default function CorrectiveList() {
  ...
  const columns = [
    ...
    { key: 'actual_cost', label: 'Cost', render: v => v ? `RM ${v}` : '-' },
    {
    ...
    <div className="space-y-4">
      ...
      {modal && (
        <Modal title={modal.type === 'create' ? 'Add Corrective Maintenance' : 'Edit CM'} onClose={() => setModal(null)} size="lg">
          <CMForm defaultValues={modal.data || { category: 'general', priority: 'medium', status: 'open', reported_date: new Date().toISOString().slice(0,10) }} onSubmit={d => modal.type === 'create' ? createMut.mutate(d) : updateMut.mutate({ id: modal.data.id, ...d })} loading={createMut.isPending || updateMut.isPending} buildings={buildings} vendors={vendors} />
        </Modal>
      )}
    </div>
  )
