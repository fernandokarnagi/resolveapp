    ...
    <div className="space-y-4">
      ...
      {modal && (
        <Modal title={modal.type === 'create' ? 'Add Schedule' : 'Edit Schedule'} onClose={() => setModal(null)} size="lg">
          <CleaningForm defaultValues={modal.data || { frequency: 'daily', status: 'scheduled' }} onSubmit={d => modal.type === 'create' ? createMut.mutate(d) : updateMut.mutate({ id: modal.data.id, ...d })} loading={createMut.isPending || updateMut.isPending} buildings={buildings} vendors={vendors} />
        </Modal>
      )}
    </div>
  )
