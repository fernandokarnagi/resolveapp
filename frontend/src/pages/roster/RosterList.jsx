...
function RosterForm({ defaultValues, onSubmit, loading, buildings, users }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-bold text-slate-800">Night Watch Duty Roster</h1></div>
      ...
      {modal && (
        <Modal title={modal.type === 'create' ? 'Add Duty Roster' : 'Edit Roster'} onClose={() => setModal(null)}>
          <RosterForm defaultValues={modal.data || { shift: 'night', status: 'scheduled', date: new Date().toISOString().slice(0,10), assigned_officer_ids: [] }} onSubmit={d => modal.type === 'create' ? createMut.mutate(d) : updateMut.mutate({ id: modal.data.id, ...d })} loading={createMut.isPending || updateMut.isPending} buildings={buildings} users={users} />
        </Modal>
      )}
    </div>
  )
