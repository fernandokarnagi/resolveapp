import { useState } from 'react'
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import { Plus, Edit, Trash2 } from 'lucide-react'
import api from '../../api/axios'
import DataTable from '../../components/DataTable'
import Modal from '../../components/Modal'
import { useForm } from 'react-hook-form'

export default function FloorsList() {
  const qc = useQueryClient()
  const [modal, setModal] = useState(null)
  const [selectedBuilding, setSelectedBuilding] = useState('')
  const [search, setSearch] = useState('')

  const { data: buildings = [] } = useQuery({
    queryKey: ['buildings'],
    queryFn: () => api.get('/api/buildings').then(r => r.data),
  })

  const { data: floors = [], isLoading } = useQuery({
    queryKey: ['floors', selectedBuilding],
    queryFn: () => selectedBuilding ? api.get(`/api/buildings/${selectedBuilding}/floors`).then(r => r.data) : Promise.resolve([]),
    enabled: !!selectedBuilding,
  })

  const createMut = useMutation({
    mutationFn: (d) => api.post(`/api/buildings/${selectedBuilding}/floors`, d),
    onSuccess: () => { qc.invalidateQueries(['floors']); setModal(null) },
  })
  const updateMut = useMutation({
    mutationFn: ({ id, ...d }) => api.put(`/api/buildings/floors/${id}`, d),
    onSuccess: () => { qc.invalidateQueries(['floors']); setModal(null) },
  })
  const deleteMut = useMutation({
    mutationFn: (id) => api.delete(`/api/buildings/floors/${id}`),
    onSuccess: () => qc.invalidateQueries(['floors']),
  })

  const filtered = floors.filter(f => f.name?.toLowerCase().includes(search.toLowerCase()))

  const columns = [
    { key: 'floor_number', label: 'Floor No' },
    { key: 'name', label: 'Name' },
    { key: 'total_units', label: 'Total Units' },
    {
      key: 'actions', label: 'Actions', render: (_, row) => (
        <div className="flex gap-2">
          <button onClick={() => setModal({ type: 'edit', data: row })} className="p-1.5 rounded hover:bg-slate-100 text-slate-500"><Edit size={15} /></button>
          <button onClick={() => { if (confirm('Delete?')) deleteMut.mutate(row.id) }} className="p-1.5 rounded hover:bg-red-50 text-red-500"><Trash2 size={15} /></button>
        </div>
      )
    }
  ]

  function FloorForm({ defaultValues, onSubmit, loading }) {
    const { register, handleSubmit } = useForm({ defaultValues })
    return (
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Floor Number *</label>
            <input type="number" {...register('floor_number', { required: true })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Name *</label>
            <input {...register('name', { required: true })} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Total Units</label>
          <input type="number" {...register('total_units')} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div className="flex justify-end">
          <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50">
            {loading ? 'Saving...' : 'Save'}
          </button>
        </div>
      </form>
    )
  }

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-800">Floors</h1>
        </div>
        <div className="flex items-center gap-3">
          <select
            value={selectedBuilding}
            onChange={e => setSelectedBuilding(e.target.value)}
            className="border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option value="">Select Building</option>
            {buildings.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
          </select>
          {selectedBuilding && (
            <button onClick={() => setModal({ type: 'create' })} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">
              <Plus size={16} /> Add Floor
            </button>
          )}
        </div>
      </div>
      <DataTable columns={columns} data={filtered} loading={isLoading && !!selectedBuilding} onSearch={setSearch} />
      {modal && (
        <Modal title={modal.type === 'create' ? 'Add Floor' : 'Edit Floor'} onClose={() => setModal(null)}>
          <FloorForm
            defaultValues={modal.data || { building_id: selectedBuilding }}
            onSubmit={(d) => modal.type === 'create' ? createMut.mutate(d) : updateMut.mutate({ id: modal.data.id, ...d })}
            loading={createMut.isPending || updateMut.isPending}
          />
        </Modal>
      )}
    </div>
  )
}
